...
writes the result to each configured target's instruction file. Each
...
- **FR-007**: The hub file write MUST be atomic (write-temp-rename).
  Symlink failures for individual targets MUST be reported without
  affecting other targets or the hub file.
